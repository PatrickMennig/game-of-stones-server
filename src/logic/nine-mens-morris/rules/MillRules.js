...
class MillRules {
    ...
    static willBeMill(move, board) {
        ...
        const isFromInMill0 = typeof fromId === 'number' ? mills[0].filter(p => p === fromId).length > 0 : false;
        const isFromInMill1 = typeof fromId === 'number' ?  mills[1].filter(p => p === fromId).length > 0 : false;
